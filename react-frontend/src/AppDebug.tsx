...
import { useDatasets, useAllCompletionDatasets } from './hooks/useComparison';
...
function AppDebug() {
  ...
  console.log('Completion datasets state:', { allCompletionDatasets, completionDatasetsError, isLoadingOutputs });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-2xl font-semibold mb-4">useAllCompletionDatasets() Hook Debug</h2>
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              <strong>Completions Count:</strong> {allCompletionDatasets?.length || 0}
            </div>
              ...
              <button
                onClick={async () => {
                  if (datasets.length > 0) {
                    ...
                    try {
                      const response = await fetch(`http://localhost:8000/api/v1/datasets/${datasetId}/completions`);
            ...
            {completionDatasetsError && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <strong className="text-red-800">Error Details:</strong>
                <pre className="text-red-700 text-sm mt-2 whitespace-pre-wrap">
                  {JSON.stringify(completionDatasetsError, null, 2)}
                </pre>
              </div>
            )}
            
            {allCompletionDatasets && allCompletionDatasets.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <strong className="text-green-800">Completion Datasets:</strong>
                <pre className="text-green-700 text-sm mt-2 whitespace-pre-wrap">
                  {JSON.stringify(allCompletionDatasets, null, 2)}
                </pre>
              </div>
            )}
          </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          ...
          <div className="text-green-700 space-y-2">
            ...
            <div className="text-sm">
              <strong>Solution:</strong> Added <code className="bg-green-100 px-1 rounded">queryClient.invalidateQueries(['all-completions'])</code> to the upload success callback in <code className="bg-green-100 px-1 rounded">useUpload.ts</code>.
            </div>
            <div className="text-sm">
